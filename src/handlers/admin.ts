...
export default class AdminHandlers {
  ...
  static async mod(ctx: Context) {
    ctx.reply(
      `Hello Captain, ${ctx.from.first_name}!\n\nWhat would you like to do today?`,
      {
        reply_markup: {
          keyboard: [
            [{ text: "📊 Get Analytics" }],
            [
              ...
              { text: "📝 Set Release Notes" },
            ],
            [
              { text: "👫 Get Target Players" },
              { text: "👫 Set Target Players" },
            ],
  ...
  static async promptRelease(ctx: Context): Promise<void> {
    ctx.reply(`Are you sure you want to release?`, {
      reply_markup: {
        inline_keyboard: [
          [
            { text: "Yes 🛰", callback_data: "release_yes" },
            { text: "No 🤔", callback_data: "release_no" },
          ],
        ],
      },
    });
  ...
  static async release(ctx: Context, confirmed: boolean) {
    if (!confirmed) {
      ...
      await ctx.editMessageText(
        `Success releases are made after multiple tries. 👨🏻‍💻`
      );
    ...
    try {
      ...
      for (const user of users) {
        await bot.api
          .sendMessage(user.id, config.releaseNote, {
            parse_mode: "HTML",
          })
          .catch((err) => {
            if (
              err.description === errors.blocked ||
              err.description === errors.cannot_initiate
            ) {
      ...
      doLog(
        `Tried sending release to: ${users.length}. Eventually realized that ${
          blockedPeeps.length
        } have blocked. That sums up to total of ${failedPeeps} failed messages.\n\nHopefully, we have delivered ${
          users.length - failedPeeps
        } messages. 🚀`
      );
    } catch (err) {
      console.log(err);
  ...
  static async getTargetPlayers(ctx: Context): Promise<void> {
    try {
      ...
      await ctx.reply(
        `👫 Current target players are: <b>${playerTypesMap[type]}</b>`,
        {
          parse_mode: "HTML",
        }
      );
      ...
      await ctx.reply(AdminHandlers.prompts.setTargetPlayers, {
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "Last Game Winners 🎉",
                callback_data: "targetPlayers_lastWinners",
              },
            ],
            [
              {
                text: "Week Backers 📅",
                callback_data: "targetPlayers_weekBackers",
              },
            ],
            [
              {
                text: "Awesome Streakers 🔥",
                callback_data: "targetPlayers_awesomeStreakers",
              },
            ],
            [
              {
                text: "Cool Gamers 🧛",
                callback_data: "targetPlayers_coolGamers",
              },
            ],
            [{ text: "Testers 👨🏻‍💻", callback_data: "targetPlayers_testers" }],
            [
              {
                text: "All Players ⛹️‍♂️",
                callback_data: "targetPlayers_allPlayers",
              },
            ],
          ],
        },
  ...
  static async setTargetPlayers(
    ...
    try {
      ...
      await ctx.editMessageText(
        `Setting target players to: <b>${playerTypesMap[value]}</b>`,
        {
          parse_mode: "HTML",
        }
      );
      ...
      await ctx.editMessageText(
        `👫 Target players set to: <b>${playerTypesMap[value]}</b>`,
        {
          parse_mode: "HTML",
        }
      );
  ...
  static async getReleaseUsersCount(ctx: Context) {
    ...
    const users = await WordleDB.getReleaseUsers(config.targetPlayers);
    await ctx.reply(
      `There are <b>${users.length}</b> users in the release list.`,
      {
        parse_mode: "HTML",
      }
    );
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "Approve ✅",
                callback_data: `testerRequest_${user.id}-approve`,
              },
            ],
            [
              {
                text: "Reject ❌",
                callback_data: `testerRequest_${user.id}-reject`,
              },
            ],
          ],
        },
  ...
  static async testerRequest(ctx: Context, value: string) {
    try {
      ...
      if (action == "approve") {
        ...
        reply =
          reply +
          `Hey, we have closely reviewd your profile and have decided to reject your tester request for now. 🙂\n\nDon't worry, you can apply again sometime later to get approved!`;
        ctx.editMessageReplyMarkup({
          reply_markup: {
            inline_keyboard: [
              [
